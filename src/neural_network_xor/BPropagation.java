...
public class BPropagation {
	...
	private BPropagation(int n, double a, int c, int number) throws IOException{
		
		System.out.println("Program begin.");
		
		File f = new File("LogFolder/log_" + n + "_" + alpha + "_" + c + "_" + number + ".csv");
		f.createNewFile();
		...
		for(int count = 0; count < c; count++){
			...
			if((count + 1) % 1000 != 0) continue;
			System.out.println(count + " in=[" + x[0] + "," + x[1] + "] out=" + round_five(z) +
					" err=" + round_five(error));
	...
	public static void main(String[] args){
		try{
			for(int i = 0; i < 10; i++){
				new BPropagation(10, 1.0, 100000, i);
			}
		}catch(IOException e){
			e.printStackTrace();
		}
