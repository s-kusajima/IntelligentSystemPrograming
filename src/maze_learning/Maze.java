...
public class Maze {
	...
	public void calcPath(){
		...
		final int INF = Integer.MAX_VALUE; // INF値
	...
	// 迷路データ
	public void setMazeBlock(int[][] m){
		
		if(m.length != 12 || m[0].length != 12){
			System.out.println("迷路の大きさが適切ではありません．");
			return;
		}
		
		for(int i = 0; i < 12; i++){
			for(int j = 0; j < 12; j++){
				this.mazeBlock[i][j] = m[i][j];
			}
		}
	}
